...
internal sealed class ButtonTemplateWriter : DisposableObjectBase
{
    ...
    /// </returns>
    public OperationalResult WriteOldFormat(ButtonTemplate template)
    {
    ...
    private void WriteStateTemplate(OperationalResult result, StateTemplate? stateTemplate)
    {
        if (result.Success)
        {
            ...
            else
            {
                ...
                byte[] imageData = ImageToBytes(stateTemplate.Image) ?? Array.Empty<byte>();
                WriteIfOk(result, Convert.ToBase64String(imageData));
    ...
    private byte[]? ImageToBytes(Image? sourceImage)
    {
