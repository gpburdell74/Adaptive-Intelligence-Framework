	...
	public sealed class ContextOperations : DisposableObjectBase
	{
		...
		public string? CreateDataDefinitionClass(string databaseName, SqlTable? table, string nameSpace, string? parentClass, bool disposable)
		{
			...
			if (table != null && dbInfo != null)
			{
				...
				if (profile != null)
				{
					...
					CodeTypeDeclaration dataDefinitionClass = CodeDomObjectFactory.CreateClass(className,
					parentClass,
					...
					if (disposable)
					{
						//  protected override void Dispose(bool disposing)
						//  {
						CodeMemberMethod disposeMethod = CodeDomObjectFactory.CreateDisposeMethod();
						//  #region Dispose Method
						disposeMethod.StartDirectives.Add(CodeDomObjectFactory.StartRegion("Dispose Method"));
						CodeDomObjectFactory.CreateDisposeMethodContent(disposeMethod, propertyList);
						dataDefinitionClass.Members.Add(disposeMethod);
						//  }
						//  #endregion
						disposeMethod.EndDirectives.Add(CodeDomObjectFactory.EndRegion());
					}
					...
					if (propertyList != null && propertyList.Count > 0)
					{
						...
						{
							CodeMemberProperty propertyDef = CodeDomObjectFactory.CreatePropertyDefinition(propertyItem);
							if (isFirst)
							{
								first = propertyDef;
								isFirst = false;
						...
						if (last != null)
						{
							last.EndDirectives.Add(CodeDomObjectFactory.EndRegion());
					...
					// Render the code.
					generatedCode = CodeDomObjectFactory.RenderCode(nameSpaceContainer);
				}
			}
		...
		public string? CreateDataAccessClass(string databaseName, SqlTable? table)
		{
			...
			if (dbInfo != null && table != null)
			{
				...
				generatedCode = builder.GenerateDataAccessClass(Adaptive.CodeDom.Languages.CSharp);
				builder.Dispose();
			}
