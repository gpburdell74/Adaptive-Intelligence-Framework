...
namespace Adaptive.Taz
{
	...
	public sealed class FileCompressionOperation : DisposableObjectBase
	{
		...
		/// <summary>
		/// Occurs when an error occurs during the fikle load operation.
		/// </summary>
		...
		public void CompressFile()
		{
			...
			{
				OnCompressionError(_fileName, ex);
				_compressedData = null;
			}
		...
		/// A value indicating the current poisition in the output file the actual compressed data is written to.
		/// </param>
		/// <returns>
		...
		private async Task ReadFileDataAsync()
		{
			...
			if (!result.Success || _fileData == null || _fileData.Length == 0)
			{
				if (result.HasExceptions)
					OnFileLoadError(_fileName, result.Exceptions[0]);
				else
