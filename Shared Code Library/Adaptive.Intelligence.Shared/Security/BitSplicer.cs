	...
	public static class BitSplicer
	{
		...
		public static byte[]? SpliceBits(byte[]? original)
		{
			...
			if ((original != null) && (original.Length > 0))
			{
				...
				byte[]? evenByteArray = CreateEvenLengthByteArray(original);

				// Create the bit arrays.
				if (evenByteArray != null)
				{
					int originalLength = evenByteArray.Length;
					int halfLength = evenByteArray.Length / 2;
					int bitArraySize = halfLength * 8;

					BitArray originalBitList = new BitArray(evenByteArray);
					BitArray leftBitList = new BitArray(bitArraySize);
					BitArray rightBitList = new BitArray(bitArraySize);
		...
		public static byte[]? UnSpliceBits(byte[]? splicedData)
		{
			...
			{
				int originalLength = splicedData.Length;
				...
				byte[] combined = new byte[originalLength];
		...
		private static byte[]? CreateEvenLengthByteArray(byte[]? original)
		{
			...
			if (original != null)
			{
				MemoryStream stream = new MemoryStream(original.Length + 4);
		...
		private static void SplitBits(BitArray? originalBitList, BitArray? leftBitList, BitArray? rightBitList)
		{
			...
			{
				int arrayIndex = 0;
				int length = originalBitList.Length;
