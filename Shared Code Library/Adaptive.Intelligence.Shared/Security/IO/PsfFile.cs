...
namespace Adaptive.Intelligence.Shared.Security.IO
{
    ...
    public sealed class PsfFile : ExceptionTrackingBase
    {
        ...
        /// If the function returns <b>false</b>, check the <see cref="ExceptionTrackingBase.Exceptions"/> property.
        /// </returns>
        ...
        private static MemoryStream PrepStream(Stream sourceStream)
        {
            ...
            var clear = reader.ReadBytes((int)sourceStream.Length);

            // Compress the data and splice the bits.
            var compressed = AdaptiveCompression.Compress(clear);
            var readyStream = new MemoryStream(BitSplicer.SpliceBits(compressed)!);
        ...
        private static MemoryStream? UncompressStream(MemoryStream compressedData)
        {
            ...
            // Un-splice the bits.
            var spliced = compressedData.ToArray();
            var unspliced = BitSplicer.UnSpliceBits(spliced);
            if (unspliced != null)
            {
                var decompressed = AdaptiveCompression.Decompress(unspliced);
